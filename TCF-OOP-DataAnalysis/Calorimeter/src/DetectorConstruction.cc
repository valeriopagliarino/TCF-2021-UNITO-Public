...
// ********************************************************************
//
/// \file electromagnetic/Calorimeter/src/DetectorConstruction.cc
...
{ 
  ...
  G4Element* F  = new G4Element("Fluorine", "H", z= 9, a= 19*g/mole);
  ...
  G4Material* CaF2 =
    new G4Material("CaF2"  , density= 3.18*g/cm3, ncomponents=2);
  CaF2->AddElement(Ca, fractionmass=0.5133);
  CaF2->AddElement(F, fractionmass=0.4867);

  G4Material* PMMA =
    new G4Material("PMMA"  , density= 1.19*g/cm3, ncomponents=3);
  PMMA->AddElement(C, fractionmass=0.6);
  PMMA->AddElement(O, fractionmass=0.32);
  PMMA->AddElement(H, fractionmass=0.08);

  G4Material* ABS =
    new G4Material("ABS"  , density= 1.1*g/cm3, ncomponents=3);
  ABS->AddElement(C, fractionmass=0.853);
  ABS->AddElement(N, fractionmass=0.0659);
  ABS->AddElement(H, fractionmass=0.0811);
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  if (fTallyNumber > 0) {
    for (G4int j=0; j<fTallyNumber; ++j) {
       ...
       fLTally[j] = new G4LogicalVolume(sTally,fTallyMaterials[j],"Tally");
...
void DetectorConstruction::SetMaterial(const G4String& materialChoice)
{
  ...
  if (pttoMaterial && pttoMaterial != fAbsorMaterial) {
    ...
    for (G4int j=0; j<fTallyNumber; ++j) {
      if(fLTally[j]) { 
        fLTally[j]->SetMaterial(fTallyMaterials[j]);
        fTallyMass[j] = fTallySize[j].x()*fTallySize[j].y()*fTallySize[j].z()
          *(fTallyMaterials[j]->GetDensity());
      }
    } 
...
void DetectorConstruction::SetTallyMaterial(G4int j, G4String matStr)
{
  G4cout << "\n\nINVOCATED SET TALLY MATERIAL " << j << "  " << matStr;
  if(j >= 0 && j < kMaxTally) {
    G4Material* mat =
        G4NistManager::Instance()->FindOrBuildMaterial(matStr);
    fTallyMaterials[j] = mat;
    G4RunManager::GetRunManager()->PhysicsHasBeenModified();
  } else {
    G4cout << "### DetectorConstruction::SetTallyNumber WARNING: wrong tally "
           << "number " << j << " is ignored" << G4endl;
